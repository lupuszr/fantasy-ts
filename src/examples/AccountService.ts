...
export class AccountService<Account, Amount, Balance> {
  constructor(
    public open: (
      no: string,
      name: string,
      openDate: Either<string, Date>
    ) => Try<Account>,
    public close: (
      account: Account,
      closeDate: Either<string, Date>
    ) => Try<Account>,
  ...
  transfer(
    ...
    return this.debit(from, amount).chain(d =>
      this.credit(to, amount).map(c => [d, c, amount])
    );
  }
}
...
export class Balance{ constructor(public amount: Amount = BigInt(0)) {}}
class Account{ 
  constructor(
    public no: String,
    public name: String,
...
export const AccountServiceImpl = new AccountService<Account, Amount, Balance>(
  ...
  (no, name, openingDate) => {
    switch(true) {
      case no === "": return failure(Error("Account no can't be blank"));
      case name === "": return failure(Error("Account name can't be blank"));
      case openingDate.isLeft(): return failure(Error("Date invalid"));
      default: return new Success(new Account(no, name, openingDate.fromRight(new Date())))
    } 
  },
  // close
